...
def get_search_provider(provider_name: str) -> SearchProvider:
    ...
    providers = {
        "bing": BingSearch,
        # "google": GoogleSearch 可扩展其他引擎
    }
    return providers.get(provider_name.lower(), DummySearch)()  # 默认返回 DummySearch
...
async def chat_completions(request: ChatRequest):
    ...
    # 如果使用联网搜索，需要修改下面stream_generate函数传入的第一个参数 messages 替换 request.messages
    # 因为其中包含了搜索结果
